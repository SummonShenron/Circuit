...
import { CheckCircle2, Clock3, CloudSun, Compass, FilePlus2, GitBranch, KeyRound, Link2, MoreHorizontal, Play, Search, Trash2, Workflow, X, XCircle } from "lucide-react";
...
const DASHBOARD_HINTS = [
  ...
  "Try one of the guided tutorials above if you're not sure where to start.",
  "Don't forget to add your env variables first.",
  "Make sure you connect to your services.",
  "Did you know you can branch workflows with a Condition block?",
  "See if you can build a workflow that retrieves and summarizes data.",
...
export default function Dashboard({ onOpen, onCreate }: DashboardProps) {
  ...
  const startTutorial = async (tutorial: "getting-started" | "chat-assistant" | "routing" | "weather-briefing" | "rag-retriever", workflowName: string) => {
    setTutorialNotice("");
    if (tutorial !== "getting-started" && tutorial !== "weather-briefing" && !secrets.some((secret) => secret.name === "CHAT_TRIGGER_TOKEN")) {
  ...
  return <main className="dashboard-shell">
    ...
    <div className="dashboard-body">
      <section className="dashboard-content"><div className="dashboard-heading"><div className="dashboard-heading-copy"><span>WORKFLOWS</span><h1>Build, run, and improve.</h1><p>Your automations are saved to your workspace and ready to pick up where you left off.</p></div>{showServiceNotice && <aside className="dashboard-service-notice" aria-label="Circuit service notice"><div className="dashboard-service-notice-copy"><strong>Circuit may need a moment</strong><p>The first request after inactivity can be slow while Circuit's backend starts. Avoid starting a new workflow until loading has completed.</p><details><summary>Learn more</summary><p>Circuit runs on cost-sensitive infrastructure and uses third-party LLMs on free tiers. This can cause longer response times or temporary unavailability during peak demand. If the app appears to hang after signing in, wait 30–60 seconds and try again.</p></details></div><button type="button" onClick={() => { setShowServiceNotice(false); window.localStorage.setItem("hideCircuitServiceNotice", "1"); }} aria-label="Dismiss Circuit service notice"><X size={15} /></button></aside>}</div>
      {error && <p className="dashboard-error">{error}</p>}
      <section className="tutorial-card"><div><span>START HERE</span><h2>Guided tutorials</h2><p>New to Circuit? Start with Getting started to learn the editor, then build a chat assistant, connect an API to an LLM, or branch a workflow with a router and condition.</p></div><div className="tutorial-card-actions"><button className="create-workflow" onClick={() => void startTutorial("getting-started", "Getting Started Tutorial")}><Compass size={16} /> Getting started</button><button className="create-workflow" onClick={() => void startTutorial("chat-assistant", "Chat Assistant Tutorial")}><Workflow size={16} /> Chat assistant</button><button className="create-workflow" onClick={() => void startTutorial("routing", "Routing Tutorial")}><GitBranch size={16} /> Routing</button><button className="create-workflow" onClick={() => void startTutorial("weather-briefing", "Weather Briefing Tutorial")}><CloudSun size={16} /> Weather briefing</button><button className="create-workflow" onClick={() => void startTutorial("rag-retriever", "RAG Retriever Tutorial")}><Search size={16} /> RAG retriever</button></div>{tutorialNotice && <p className="dashboard-error">{tutorialNotice}</p>}</section>
    ...
    <div className="dashboard-mascot"><PatchyEmptyState tab="open" compact hints={DASHBOARD_HINTS} hintIntervalMs={3_000} /></div>
  </main>;
}