...
const blockHelp: Record<Kind, HelpTopic> = {
  ...
  google_drive_update: { title: "Update Google Drive File block", what: "Replaces the content of an existing Google Drive file using its file ID and your Google Workspace connection. Check \"Append\" to add to the existing content instead of overwriting it.", when: "Use it when a recurring workflow should keep one report or document up to date, or build up a running log.", example: "File ID: 1abc..., Content: {{generate_briefing.briefing}}, Append: on" },
  google_drive_read: { title: "Read Google Drive File block", what: "Reads an existing Google Drive file's text content using its file ID and your Google Workspace connection. Google Docs, Sheets, and Slides are exported as text automatically.", when: "Use it to pull a report, log, or data file into an LLM or Transform block.", example: "File ID: 1abc..., then pass {{report.text}} to an LLM block." },
  ...
  schedule: { title: "Schedule / event trigger", what: "Starts a workflow on a recurring timer, receives a signed JSON event from a CLI, frontend, or another backend, or watches a Google Drive folder for new files.", when: "Use schedule mode for recurring work. Use event mode to expose a workflow as a headless API. Use Drive watch mode to run the workflow once per new file dropped into a folder.", example: "Drive watch: Folder ID 1abc..., checked every 5 minutes; each run gets drive_file_id, drive_file_name, drive_file_mime_type as inputs." },
  ...
  current_datetime: { title: "Current Date & Time block", what: "Returns the current date, time, and weekday in a chosen timezone (UTC, Central, Eastern, and more).", when: "Use it to timestamp a report, label a file, or make time-of-day decisions in a workflow.", example: "Timezone: Central, Output key: now -> {{now.date}}, {{now.time}}" },
};
const panelHelp = {
  ...
  const Component = {
    ...
    github_repository: GitBranch,
      ...
      google_drive: HardDrive,
        ...
        google_drive_read: HardDrive,
        current_datetime: CalendarClock,
...
function outputKeys(node: FlowNode) {
  ...
  if (node.data.kind === "google_drive_update")
    return [String(node.data.config.output_key ?? "drive_file")];
  if (node.data.kind === "google_drive_read")
    return [String(node.data.config.output_key ?? "drive_file")];
  if (node.data.kind === "current_datetime")
    return [String(node.data.config.output_key ?? "now")];
...
function variablesFor(
  ...
  const driveWatchVariables: Variable[] = nodes.some((node) => node.data.kind === "schedule" && node.data.config.trigger_mode === "drive_watch")
    ? [
        { token: "{{input.drive_file_id}}", label: "Drive watch trigger", detail: "New file's Drive file ID" },
        { token: "{{input.drive_file_name}}", label: "Drive watch trigger", detail: "New file's name" },
        { token: "{{input.drive_file_mime_type}}", label: "Drive watch trigger", detail: "New file's MIME type" },
      ]
    : [];
  return [...inputVariables, ...driveWatchVariables, ...outputVariables];
}
...
function ScheduleForm({ config, bodyOptions, returnTo, onChange }: { config: Record<string, unknown>; bodyOptions: FlowNode[]; returnTo: string; onChange: (config: Record<string, unknown>) => void }) {
  ...
  void errorHandlers;
  const triggerMode = String(config.trigger_mode ?? "schedule");
    return <div className="node-form"><p className="form-hint">The scheduler checks enabled schedules every 15 seconds. Event mode is configuration-only until an event source is connected.</p><label className="required-input"><input type="checkbox" checked={Boolean(config.enabled ?? true)} onChange={(event) => update("enabled", event.target.checked)} /> Enabled</label><label>Trigger mode<select value={triggerMode} onChange={(event) => update("trigger_mode", event.target.value)}><option value="schedule">Scheduled timer</option><option value="event">External event</option><option value="drive_watch">New file in Google Drive folder</option></select></label>{triggerMode === "event" ? <label>Event name<input value={String(config.event_name ?? "")} onChange={(event) => update("event_name", event.target.value)} placeholder="github.issue.created" /></label> : triggerMode === "drive_watch" ? <><p className="form-hint">Runs the workflow once for every new file that appears in the folder. Find the folder ID in its Drive URL: drive.google.com/drive/folders/&lt;folder id&gt;. Each run receives drive_file_id, drive_file_name, and drive_file_mime_type as inputs — pass drive_file_id to a Read Drive file block to fetch its content.</p><ConnectionPicker value={String(config.drive_connection_id ?? "")} returnTo={returnTo} onChange={(connectionId) => update("drive_connection_id", connectionId || undefined)} /><label>Folder ID<input value={String(config.drive_folder_id ?? "")} onChange={(event) => update("drive_folder_id", event.target.value)} placeholder="1abc..." /></label><label>Check frequency<select value={String(config.interval ?? "hourly")} onChange={(event) => update("interval", event.target.value)}><option value="5_minutes">Every 5 minutes</option><option value="hourly">Every hour</option><option value="daily">Every day</option><option value="weekly">Every week</option></select></label></> : <><label>Frequency<select value={String(config.interval ?? "hourly")} onChange={(event) => update("interval", event.target.value)}><option value="5_minutes">Every 5 minutes</option><option value="hourly">Every hour</option><option value="daily">Every day</option><option value="weekly">Weekly</option></select></label><label>Specific time (optional)<input type="time" value={String(config.time_of_day ?? "")} onChange={(event) => update("time_of_day", event.target.value || undefined)} /></label><label>Timezone<select value={String(config.timezone ?? "UTC")} onChange={(event) => update("timezone", event.target.value)}><option value="UTC">UTC</option><option value="America/New_York">Eastern</option><option value="America/Chicago">Central</option><option value="America/Denver">Mountain</option><option value="America/Los_Angeles">Pacific</option><option value="Europe/London">London</option><option value="Europe/Paris">Paris</option><option value="Asia/Tokyo">Tokyo</option></select></label>{config.interval === "weekly" && <label>Every N weeks<input type="number" min="1" max="52" value={Number(config.weeks_interval ?? 1)} onChange={(event) => update("weeks_interval", Number(event.target.value))} /></label>}<label>Days of week<select multiple value={days.map(String)} onChange={(event) => update("days_of_week", Array.from(event.target.selectedOptions).map((option) => Number(option.value)))}><option value="0">Monday</option><option value="1">Tuesday</option><option value="2">Wednesday</option><option value="3">Thursday</option><option value="4">Friday</option><option value="5">Saturday</option><option value="6">Sunday</option></select></label></>}</div>;
}
...
function NodeForm({
  ...
  if (kind === "schedule") return <><ScheduleForm config={config} bodyOptions={bodyOptions} returnTo={returnTo} onChange={onChange} /><div className="node-form"><ScheduleResponseField config={config} bodyOptions={bodyOptions} onChange={onChange} /><ScheduleErrorHandlerField config={config} bodyOptions={bodyOptions} onChange={onChange} /></div></>;
  ...
  if (kind === "current_datetime") return <div className="node-form"><p className="form-hint">Returns the current date, time, and weekday in the selected timezone.</p><label>Timezone<select value={String(config.timezone ?? "UTC")} onChange={(event) => update("timezone", event.target.value)}><option value="UTC">UTC</option><option value="America/New_York">Eastern</option><option value="America/Chicago">Central</option><option value="America/Denver">Mountain</option><option value="America/Los_Angeles">Pacific</option><option value="Europe/London">London</option><option value="Europe/Paris">Paris</option><option value="Asia/Tokyo">Tokyo</option></select></label><label>Output key<input value={String(config.output_key ?? "now")} onChange={(event) => update("output_key", event.target.value)} /></label></div>;
  if (kind === "transform") {
  ...
  if (kind === "google_drive_update") return <div className="node-form"><p className="form-hint">Updates an existing Google Drive file. Find the file ID in its Drive URL.</p><Field {...field("google_drive.connection")}><ConnectionPicker value={String(config.connection_id ?? "")} returnTo={returnTo} onChange={(connectionId) => update("connection_id", connectionId || undefined)} /></Field><label>File ID<input value={String(config.file_id ?? "")} onChange={(event) => update("file_id", event.target.value)} placeholder="1abc..." /></label><label>Content<textarea rows={8} value={String(config.content ?? "")} onChange={(event) => update("content", event.target.value)} /><TemplateWarnings value={String(config.content ?? "")} variables={variables} /><VariablePicker variables={variables} onInsert={(token) => update("content", `${String(config.content ?? "")}${token}`)} /></label><label className="required-input"><input type="checkbox" checked={Boolean(config.append)} onChange={(event) => update("append", event.target.checked)} /> Append to existing content (instead of replacing it)</label><label>MIME type<input value={String(config.mime_type ?? "text/plain")} onChange={(event) => update("mime_type", event.target.value)} placeholder="text/markdown" /></label><label>Output key<input value={String(config.output_key ?? "drive_file")} onChange={(event) => update("output_key", event.target.value)} /></label><Field {...field("google_drive.retry")}><RetryFields value={config.retry as Record<string, unknown> | undefined} onChange={(retry) => update("retry", retry)} /></Field></div>;
  if (kind === "google_drive_read") return <div className="node-form"><p className="form-hint">Reads an existing Google Drive file's content. Type a literal file ID from its Drive URL, or use the variable picker to insert {"{{input.drive_file_id}}"} when this workflow is triggered by a Drive watch schedule node. Google Docs, Sheets, and Slides are exported as text automatically.</p><Field {...field("google_drive.connection")}><ConnectionPicker value={String(config.connection_id ?? "")} returnTo={returnTo} onChange={(connectionId) => update("connection_id", connectionId || undefined)} /></Field><label>File ID<input value={String(config.file_id ?? "")} onChange={(event) => update("file_id", event.target.value)} placeholder="1abc... or {{input.drive_file_id}}" /><TemplateWarnings value={String(config.file_id ?? "")} variables={variables} /><VariablePicker variables={variables} onInsert={(token) => update("file_id", `${String(config.file_id ?? "")}${token}`)} /></label><label className="required-input"><input type="checkbox" checked={Boolean(config.parse_json)} onChange={(event) => update("parse_json", event.target.checked)} /> Parse as JSON</label><label>Output key<input value={String(config.output_key ?? "drive_file")} onChange={(event) => update("output_key", event.target.value)} /></label><Field {...field("google_drive.retry")}><RetryFields value={config.retry as Record<string, unknown> | undefined} onChange={(retry) => update("retry", retry)} /></Field></div>;
